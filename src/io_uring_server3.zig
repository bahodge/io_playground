...
pub const IO = struct {
                    ...
                    const result: CancelError!void = result: {
                        if (completion.result < 0) {
                            break :result switch (@as(posix.E, @enumFromInt(-completion.result))) {
                                ...
                                // SQE is invalid.
                                .INVAL => unreachable,
                                else => |errno| unexpected_errno("cancel", errno),
                            };
                        }
                    };
        ...
        switch (kind) {
            ...
            .block_device => {
                ...
                switch (os.linux.E.init(os.linux.ioctl(
                    ...
                    else => |err| return unexpected_errno("open_file:ioctl", err),
                }
    ...
    fn fs_is_tmpfs(dir_fd: fd_t) !bool {
        ...
        while (true) {
            ...
            switch (os.linux.E.init(res)) {
                ...
                .INTR => continue,
                else => |err| return unexpected_errno("fs_is_tmpfs", err),
            }
        }
    }
    ...
    fn fs_supports_direct_io(dir_fd: fd_t) !bool {
        ...
        _ = array_print(16, &cookie, "{0x}", .{std.crypto.random.int(u64)});
        ...
        while (true) {
            ...
            switch (os.linux.E.init(res)) {
                ...
                .INTR => continue,
                .INVAL => return false,
                else => |err| return unexpected_errno("fs_supports_direct_io", err),
            }
        }
    }
    ...
    fn fs_allocate(fd: fd_t, size: u64) !void {
        ...
        while (true) {
            ...
            switch (os.linux.E.init(rc)) {
                ...
                else => |errno| return unexpected_errno("fs_allocate", errno),
            }
        }
    }
};
...
const Server = struct {
    ...
    pub fn write_callback(self: *Self, comp: *IO.Completion, res: IO.WriteError!usize) void {
        ...
        std.debug.print("wrote some stuff\n", .{});
    }
};
