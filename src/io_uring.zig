...
fn readNBytes(
    ...
    var written: usize = 0;
    while (data.len < n) {
        const nwritten = try file.read(buf);
...
const Benchmark = struct {
    ...
    fn stop(b: *Benchmark) void {
        ...
        const in = readNBytes(b.allocator, OUT_FILE, b.data.len) catch unreachable;
...
fn benchmarkIOUringNEntries(
    ...
    var ring = try std.os.linux.IoUring.init(nEntries, 0);
    defer ring.deinit();
